...
public class DataRetriever {
    ...
    public Order saveOrder(Order orderToSave) {
        try (Connection conn = new DBConnection().getConnection()) {
            ...
            // Valeurs par défaut (compatibilité anciens tests)
            ...
            // VÉRIFICATION: Si c'est une commande existante, on vérifie qu'elle n'est pas déjà livrée
            if (orderToSave.getId() != 0) {
                try {
                    ...
                    // Si la commande n'existe pas (nouvelle), on continue
            ...
            String insertOrderSql =
                    "INSERT INTO \"order\" (id, reference, creation_datetime, type, status) VALUES (?, ?, ?, ?, ?) RETURNING id";

            try (PreparedStatement ps = conn.prepareStatement(insertOrderSql)) {
                orderId = getNextSerialValue(conn, "order", "id");
    ...
    public Dish saveDish(Dish dishToSave) {
        try (Connection conn = new DBConnection().getConnection()) {
            ...
            // Cas UPDATE - vérifier si l'ID existe ET n'est pas 0
            if (dishToSave.getId() != null && dishToSave.getId() != 0) {
                String updateSql = "UPDATE dish SET name=?, dish_type=?, selling_price=? WHERE id=?";
                try (PreparedStatement ps = conn.prepareStatement(updateSql)) {
            ...
            String insertSql = "INSERT INTO dish (id, name, dish_type, selling_price) VALUES (?, ?, ?, ?)";
    ...
    public Ingredient saveIngredient(Ingredient ingredientToSave) {
        try (Connection conn = new DBConnection().getConnection()) {
            ...
            // Cas UPDATE - vérifier si l'ID existe ET n'est pas 0
            if (ingredientToSave.getId() != null && ingredientToSave.getId() != 0) {
